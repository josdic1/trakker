...
function processRelationships(entities, relationships) {
  ...
  relationships.forEach(rel => {
    ...
    if (entity1HasMany === null || entity2HasMany === null) return;  // Skip incomplete
    if (!entity1HasMany && !entity2HasMany) return;  // Skip no-rel
    ...
    entity.fields.forEach(field => {
      if (field.name) {
        ...
        if (field.name.toLowerCase() === 'name') {
          ...
          columnArgs = `nullable=True`;  // Default for other fields
        }
